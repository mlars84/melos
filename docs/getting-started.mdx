...
Different projects might use different versions of Melos. To ensure everyone
working on the project (as well as CI) is using the same version of Melos, a
dependency on Melos has to be added to the `pubspec.yaml` file at the root of
the repository. The globally installed version of Melos will switch to the
version specified in the `pubspec.yaml` file, if both versions are not the same.

If you don't have a `pubspec.yaml` file at the prository root yet, create one
now:
